...
const LongBlogCard = ({ blog }) => {
  ...
  const handleOptionClick = async (option) => {
    if (option === "edit") {
      ...
      try {
        const response = await axios.delete(`http://127.0.0.1:8787/api/v1/blog/${blog?.id}`,{
          headers: {
    ...
    <div className="flex">
      <Link to={`/blogs/${blog.id}`} className='relative'>
        <div className='md:w-[290px] md:h-[185px] w-[190px] h-[90px]'>
          <img src={blog.images[0]} alt="burger" className="w-[190px] h-[90px] md:w-[290px] md:h-[185px]  lg:w-[290px]  rounded-t-[21px] object-cover" />
        </div>
      </Link>
      <Link to={`/blogs/${blog.id}`} className='relative'>
        <div className="ml-4 flex flex-col justify-between overflow-auto">
          <div className='text-[20px] md:pt-0 lg:pt-0 md:text-[25px] lg:text-[30px] text-[#233780] inter font-[600] '>
            <p className='' dangerouslySetInnerHTML={{ __html: blog.title }}></p>
          </div>
